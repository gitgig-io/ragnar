...
import {BountyUtils} from "./libraries/BountyUtils.sol";

contract BountiesV2 is
  EIP712,
  Pausable,
  AccessControlDefaultAdminRules
{
  ...
  event ConfigChange(address configContract, address maintainerFeesContract);
  ...
  uint64 public constant RECLAIM_START = 0 days;
  ...
  address public maintainerFeesContract;

  constructor(address _configContract, address _maintainerFeesContract, address _custodian, address _finance)
    Pausable()
    ...
    EIP712("GitGigBounties", "1")
  {
    ...
    maintainerFeesContract = _maintainerFeesContract;
    emit ConfigChange(configContract, maintainerFeesContract);
  }
  ...
  function setMaintainerFeesContract(address _maintainerFeesContract) external onlyRole(CUSTODIAN_ROLE) {
    maintainerFeesContract = _maintainerFeesContract;
    emit ConfigChange(configContract, maintainerFeesContract);
  }
  ...
  function maintainerClaim(
    ...
    for (uint256 index = 0; index < _bountyTokens.length; index++) {
      ...
      uint256 _amount = BountyUtils.maintainerClaimAmount(
        _platformId, 
        _repoId, 
        _issueId, 
        _bountyTokens[index],
        maintainerFeesContract,
        configContract,
        bounties
      );
  ...
  function contributorClaim(
    ...
    for (
      ...
      if (!_platformUser.platformId.eq(_platformId)) {
        continue;
      }
  ...
  function _contributorClaim(
    ...
    for (uint256 i = 0; i < _bountyTokens.length; i++) {
      ...
      uint8 _remainingClaims = BountyUtils.claimsRemaining(
        bounties[_platformId][_repoId][_issueId],
        claimed[_platformId][_repoId][_issueId],
        resolvers[_platformId][_repoId][_issueId],
        _tokenContract
  ...
  function _isUnclaimedResolver(
    ...
    return BountyUtils.isUnclaimedResolver(
      _supportedTokens,
      _platformId,
      _claimed,
      _bounties,
